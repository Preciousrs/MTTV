...
class MultimodalBertEncoder(nn.Module):
    def __init__(self, args):
        ...
        # Only use RegionVisualFeatureEncoder for entity-level visual features
    ...
    def forward(self, input_txt, attention_mask, segment, region_image):
        bsz = input_txt.size(0)
        img_embed_seq_length = region_image.size(1) + 2  # +2 for CLS and SEP Token
        ...
        img_tok = (
            torch.cat([
                torch.LongTensor(input_txt.size(0), img_embed_seq_length).fill_(1)
            ], dim=1).cuda()
        )

        img = self.img_encoder(region_image)
...
class MTTV(nn.Module):
    ...
    def forward(self, txt, mask, segment, regions):
        x = self.enc(txt, mask, segment, regions)
        x = self.clf(x)
        return x
