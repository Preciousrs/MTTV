...
from pytorch_pretrained_bert.modeling import BertModel, BertConfig, BertEncoder, BertEmbeddings
import copy
from torch.nn.parameter import Parameter

from mttv.models.image import GlobalVisualFeatureEncoder
...
class ImageBertEmbeddings(nn.Module):
    ...
    def forward(self, input_imgs, token_type_ids):
        ...
        seq_length = input_imgs.size(1) + 2   # +2 for CLS and SEP Token
...
class MultimodalBertEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.txt_embeddings = copy.deepcopy(bert.embeddings)
        self.img_embeddings = ImageBertEmbeddings(args, self.txt_embeddings)

        # Only use GlobalVisualFeatureEncoder for Global-level visual features
        self.img_encoder = GlobalVisualFeatureEncoder(args)

        # Set the number of transformer encoder layers
        ...
        for i in range(encoder_layer_num):
            self.encoder.layer[i] = copy.deepcopy(bert.encoder.layer[i])

        self.pooler = bert.pooler
    
    def forward(self, input_txt, attention_mask, segment, global_image):
        bsz = input_txt.size(0)
        img_embed_seq_length = global_image.size(1) + 2  # +2 for CLS and SEP Token
        ...
        img = self.img_encoder(global_image)
